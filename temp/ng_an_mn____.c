...
/*  Ver 3.03 , Mistakes correction June(May)                2001 Moscow     */
/****************************************************************************/
						  ...
						  FLOAT s_engIncr, FLOAT flwSecs)
{
		 ...
		 PFlow[flow_no].AccVolMonth     += (DOUBLE)volIncr;
		 PFlow[flow_no].AccVolThisYear  += (DOUBLE)volIncr;
   PFlow[flow_no].AccI_EngToday   += i_engIncr;
   PFlow[flow_no].AccI_EngMonth   += i_engIncr;
   PFlow[flow_no].AccI_EngThisYear+= (DOUBLE)i_engIncr;
   PFlow[flow_no].AccS_EngToday   += s_engIncr;
   PFlow[flow_no].AccS_EngMonth   += s_engIncr;
   PFlow[flow_no].AccS_EngThisYear+= (DOUBLE)s_engIncr;
...
void CalculateFlowRate (void)
{
  ...
  if (Annubar_10||Annubar_15||Annubar_25||Annubar_35||Annubar_45)
	{
	if (ORIFMATCODE<4)ORIFMATCODE=4;

	if (Annubar_10)
		{ 
		PIPEMATCODE = (ORIFMATCODE==4)?(3):(ORIFMATCODE);
		if (fabs(ORIFDSHOP- 4.3942)>1) ORIFDSHOP =  4.3942;
		};
	if ( (Annubar_15)&&(fabs(ORIFDSHOP- 9.2710)>2) ) ORIFDSHOP =  9.2710;
	if ( (Annubar_25)&&(fabs(ORIFDSHOP-21.7424)>3) ) ORIFDSHOP = 21.7424;
	if ( (Annubar_35)&&(fabs(ORIFDSHOP-31.3690)>4) ) ORIFDSHOP = 31.3690;
	if ( (Annubar_45)&&(fabs(ORIFDSHOP-49.5300)>5) ) ORIFDSHOP = 49.5300;
	};
...
{     
	...
	if (Annubar_35)								/*Annubar koeffitsient for 35/36*/
		{musor = (1.0-1.2075*B); ROUGHNESS = musor / sqrt(1.0+1.3416*musor*musor);}; 
	
	if (Annubar_45)								/*Annubar koeffitsient for 45/46*/
		{musor = (1.0-1.24*B); ROUGHNESS = musor / sqrt(1.0+1.2613*musor*musor);}; 
	

	EXPFACTOR = 1.0 -((1.0 - B)*(1.0 - B)*0.31424-0.09484)*DP_AVERAGE/
		           (ISENTREXP * PF_AVERAGE);  		/* Algoritm (22) */
...
get_param(17,flow_no,2,(UINT8 *)&musor);  
get_param(17,flow_no,3,(UINT8 *)&P_z);  
get_param(17,flow_no,4,(UINT8 *)&fi_z);  
T_z=musor+273.15;
...
hvost_Aj  = 0.001 * PF_AVERAGE / pow(TF_AVERAGE,2.5);
hvost_Ajc = 0.001 * BASEPRESS  / pow((BASETEMP+273.15),2.5);
hvost_Bj  = 0.001 * PF_AVERAGE / TF_AVERAGE;
...
/*
set_param(17,flow_no,7,(UINT8 *)&P_sat_steam); 
set_param(17,flow_no,8,(UINT8 *)&RO_sat_steam);  

set_param(17,flow_no,9,(UINT8 *)&P_sat_steam_z);  
set_param(17,flow_no,10,(UINT8 *)&RO_sat_steam_z);  
*/
	...
	dry_dens = ENTERED_DENS_BASE * (BASETEMP + 273.15)*
	 (PF_AVERAGE - fi*P_sat_steam/1000)/(BASEPRESS * TF_AVERAGE * COEFFZ);/* Algoritm (16) */

	wet_dens = dry_dens + fi * RO_sat_steam ;				/* Algoritm (17) */
	
/*set_param(17,flow_no,12,(UINT8 *)&fi);  
set_param(17,flow_no,13,(UINT8 *)&dry_dens );
set_param(17,flow_no,14,(UINT8 *)&wet_dens );
*/
